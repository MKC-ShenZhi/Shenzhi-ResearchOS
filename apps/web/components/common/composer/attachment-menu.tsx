...
export function AttachmentMenu({
  ...
  maxFiles?: number;
  maxSizeMb?: number;
  /** 传入时「上传本地文件夹」改走工作区（保留相对路径），不再当普通附件解析。 */
  ...
  const pickFolder = (files: FileList | null) => {
    ...
    if (onWorkspaceFolder) {
      // 工作区模式：保留 webkitRelativePath，交给调用方上传到服务端工作区
      const list = Array.from(files).map((file) => ({
        file,
        relativePath: file.webkitRelativePath || file.name,
      }));
      onWorkspaceFolder(list);
      setOpen(false);
      return;
    }
    void pickFiles(files); // 兼容旧行为（Chat）：文件夹内文件逐个按附件解析
  };
