...
import { BookOpenText, Compass, Download, FileDown, FileSearch, FileText, Loader2, MessageCircleQuestion, Quote, Sparkles, Zap } from "lucide-react";
...
type StoredSegment = { kind: "reasoning"; text: string } | { kind: "tool"; tool: Activity };

/**
 * 过程流 → 会话存储的既有形状（不引入新格式）。
 *
 * 存储里 `process` 本来就是"交错条目数组"，正好对应我们的 entries；工具条目原样落盘，
 * 思考条目落成 reasoning、正文条目落成 reasoning 之外的文本条目——旧版本读到时按思考渲染，
 * 新版本读回来仍能还原交错顺序。
 */
function toStoredProcess(stream: StreamState): StoredSegment[] | undefined {
  ...
  for (const entry of stream.entries) {
    ...
    if (entry.text.trim()) segments.push({ kind: "reasoning", text: entry.text });
  }
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      await streamAgentRun({
        ...
        onToolEnd: (toolCallId, isError, durationMs) => patchAssistant((turn) => ({
          ...turn,
          // 回填到同一个工具对象：工具行据此从"转圈"变"完成"
          stream: applyToolEnd(turn.stream, toolCallId, { done: true, isError, durationMs }),
        })),
