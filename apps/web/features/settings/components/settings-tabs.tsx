...
export function SettingsTabs() {
  ...
  return <SettingsTabsForIdentity key={session?.user.id ?? "anonymous"} />;
}

function SettingsTabsForIdentity() {
  ...
  return <>
    ...
    <Tabs value={active} onValueChange={(value) => router.replace(`/settings?tab=${value}`, { scroll: false })}>
      ...
      <TabsContent id="settings-panel-profile" aria-labelledby="settings-tab-profile" value="profile" className="mt-6"><ProfilePanel account={<AccountSection key={session?.user.id ?? "anonymous"} locale={settings.locale} />} user={session?.user ?? null} locale={settings.locale} themeMode={settings.themeMode} loading={settings.loading} notice={settings.notice} error={settings.error} onLocaleChange={(value) => void settings.setLocale(value)} onThemeChange={(value) => void settings.setTheme(value)} /></TabsContent>
