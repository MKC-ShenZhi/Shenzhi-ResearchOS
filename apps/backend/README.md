...
# Windows 本地持久化开发：先启动项目专用 PostgreSQL 5432 实例。
# PowerShell/CMD:
...
```text
api/chat.py → services/chat/service.py → services/chat/{repository,attachments,grounding,...}
→ integrations/{llm,web_search} → 外部 Provider
```
