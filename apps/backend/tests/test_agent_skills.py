...
class StoreTests(unittest.TestCase):
    ...
    def test_default_store_works_without_business_skills(self):
        """无业务 Skill 时可启动；之后放入第一方 Skill 可即时发现。"""
        from app.services.agent import skills

        builtin = self.base / 'builtin'
        builtin.mkdir()
        with patch.dict(os.environ, {'SKILLS_VENDOR': 'none'}), \
                patch.object(skills, 'BUILTIN_SKILLS_DIR', builtin):
            self.assertEqual(default_store().names(), [])
            make_skill(builtin, 'sample')
            store = default_store()
            self.assertEqual(store.names(), ['sample'])
            self.assertTrue(store.get('sample').executable)
