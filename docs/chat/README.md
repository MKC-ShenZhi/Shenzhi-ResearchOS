...
app/agents/page.tsx 或 app/agents/ask/page.tsx（URL 兼容）
  ...
  → FastAPI api/{chat,uploads}
      ├─ api/_support/upload_reader（HTTP multipart transport）
      ├─ services/chat/{service,grounding,knowledge_query,attachments}
      ├─ services/chat/{repository,postgres_repository}（Memory / PostgreSQL）
      ├─ services/uploads/parser（PDF / TXT / Markdown）
      ├─ integrations/llm/provider（OpenAI-compatible HTTP）
      └─ integrations/web_search/provider（Tavily → SearXNG）
```
...
智能搜索开启时，`services/chat/service.py` 只对 Knowledge 检索 query 做少量确定性的问答壳归一化
（不调用 LLM、不翻译、不改写原始消息），然后调用现有
`services/knowledge/service.py → integrations/knowledge` Capability。SearchResponse 经
`services/chat/grounding.py` 中的 `KnowledgeContextBuilder` 过滤无 abstract 的结果、按现有返回顺序
